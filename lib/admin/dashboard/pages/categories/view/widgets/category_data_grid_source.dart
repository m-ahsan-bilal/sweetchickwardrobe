...
class CategoryGridSource extends DataGridSource {
  /// Creates the order data source class with required details.
  CategoryGridSource({this.model, required this.isWebOrDesktop}) {
  ...
  List<CategoryModel> tickets = <CategoryModel>[];
  ...
  void buildDataGridRows(UserVM vm) {
    if (vm.selectedIndex == 0) {
      dataGridRows = tickets.where((element) => element.categoryName.toString().isCaseInsensitiveContains(vm.searchController.text)).toList().map<DataGridRow>((CategoryModel model) {
        return DataGridRow(cells: <DataGridCell>[
          DataGridCell<CategoryModel>(columnName: 'sr_no', value: model),
          DataGridCell<CategoryModel>(columnName: 'image', value: model),
          DataGridCell<CategoryModel>(columnName: 'name', value: model),
          DataGridCell<CategoryModel>(columnName: 'Description', value: model),
          DataGridCell<CategoryModel>(columnName: 'created_at', value: model),
          DataGridCell<CategoryModel>(columnName: 'status', value: model),
          DataGridCell<CategoryModel>(columnName: 'action', value: model),
        ]);
      }).toList();
    } else {
      dataGridRows = tickets
          .where((element) => element.categoryName.toString().isCaseInsensitiveContains(vm.searchController.text))
          .where((element) => element.status == GlobalFunction.getUserStatus(selectedIndex: vm.selectedIndex))
          .toList()
          .map<DataGridRow>((CategoryModel model) {
        return DataGridRow(cells: <DataGridCell>[
          DataGridCell<CategoryModel>(columnName: 'sr_no', value: model),
          DataGridCell<CategoryModel>(columnName: 'image', value: model),
          DataGridCell<CategoryModel>(columnName: 'name', value: model),
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    ...
    // }
    if (isWebOrDesktop) {
      CategoryModel model = row.getCells()[0].value;
      DateTime now = DateTime.now();
      var vm = Provider.of<CategoryVM>(navigatorKey.currentState!.context, listen: false);
          ...
          onSelected: (val) async {
            ...
            switch (val) {
              case 0:
                ...
                await updateStatus(model.categoryId ?? "", UserStatus.ACTIVE.index);
                break;
              case 1:
                // model.status=UserStatusEnum.blocked;
                await updateStatus(model.categoryId ?? "", UserStatus.BLOCKED.index);
                break;
            }
            await vm.getCategories();
            categoryDataSource = CategoryGridSource(isWebOrDesktop: isWebOrDesktop);
            Get.forceAppUpdate();
          },
          itemBuilder: (context) => [
  ...
  List<CategoryModel> getLists() {
    return Provider.of<CategoryVM>(navigatorKey.currentState!.context, listen: false).categoryList;
  }

  Future<void> updateStatus(String id, int status) async {
    try {
      await FBCollections.categories.doc(id).update({"status": status});
    } catch (e) {
      debugPrintStack();
    }
  }
