        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(labelText: 'Discount Available (true/false)'),
            ),
            TextField(
            ...
            TextField(
              ...
              decoration: InputDecoration(labelText: 'Additional Info - Brand'),
            ),
            TextField(
              controller: _additionalInfoMaterialController,
              decoration: InputDecoration(labelText: 'Additional Info - Material'),
            ),
            TextField(
            ...
            TextField(
              ...
              decoration: InputDecoration(labelText: 'Shipping Estimated Delivery'),
            ),
            TextField(
              controller: _shippingFreeShippingController,
              decoration: InputDecoration(labelText: 'Free Shipping (true/false)'),
            ),
          ],
        ),
      ),
      actions: [
        ...
        ElevatedButton(
          onPressed: () {
            ...
            final sizes = _sizesController.text.split(',').map((s) => s.trim()).toList();
            final imageUrl = _imageUrlController.text.split(',').map((s) => s.trim()).toList();
            final stockQuantity = int.tryParse(_stockQuantityController.text) ?? 0;
            final discountAvailable = _discountAvailableController.text.toLowerCase() == 'true';
            final discountPercentage = int.tryParse(_discountPercentageController.text) ?? 0;
            final additionalInfoBrand = _additionalInfoBrandController.text;
            final additionalInfoMaterial = _additionalInfoMaterialController.text;
            final ratingAverage = double.tryParse(_ratingAverageController.text) ?? 0.0;
            final ratingReviews = int.tryParse(_ratingReviewsController.text) ?? 0;
            final shippingEstimatedDelivery = _shippingEstimatedDeliveryController.text;
            final shippingFreeShipping = _shippingFreeShippingController.text.toLowerCase() == 'true';

            if (name.isNotEmpty && price > 0 && categoryId.isNotEmpty) {
              Navigator.of(context).pop({
                'name': name,
